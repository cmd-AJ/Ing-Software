async function deleteJobFromAvailableWithID(hiringId: Number | string) {
    ...
    try {
        const response = await fetch(`http://${import.meta.env.VITE_API_HOSTI}:${import.meta.env.VITE_PORTI}/api/contacts/hirings/delete/${hiringId}`, {
            method: 'DELETE',
            headers: {
...
async function insertHiringIntoFinishedJobs(deletedJob: any) {
    ...
    try {
        const response = await fetch(`http://${import.meta.env.VITE_API_HOSTI}:${import.meta.env.VITE_PORTI}/api/contacts/hire/complete`, {
            method: 'POST',
            headers: {
                'api-key': import.meta.env.VITE_API_KEY,
                'Content-Type': 'application/json',
